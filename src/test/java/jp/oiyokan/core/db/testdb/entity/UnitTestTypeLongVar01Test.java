 ...
 * 通常 $filterも交えて確認.
 */
class UnitTestTypeLongVar01Test {
    @Test
    void test01() throws Exception {
        ...
        assertEquals("\"ABCDXYZ\"", OiyokanTestUtil.getValueFromResultByKey(result, "StringLongVar1"));
        assertEquals(201, resp.getStatusCode());

        resp = OiyokanTestUtil.callGet("/ODataTest1",
                "$select=ID,StringLongVar1 &$filter=ID eq " + idString + " and StringLongVar1 eq 'ABCDXYZ'");
        result = OiyokanTestUtil.stream2String(resp.getContent());
        assertEquals("{\"@odata.context\":\"$metadata#ODataTest1\",\"value\":[{\"ID\":" + idString //
                + ",\"StringLongVar1\":\"ABCDXYZ\"}]}", result);
